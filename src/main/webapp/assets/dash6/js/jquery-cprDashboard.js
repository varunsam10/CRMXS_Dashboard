					...
					.widget(
							"mn.sDashboard",
							{
								...
								_createView : function() {
									...
									$("body")
											.append(
													"<div class='sDashboard-overlay'></div>");
									...
									for (i = 0; i < _dashboardData.length; i++) {
										var widget = this
												._constructWidget(_dashboardData[i]);
									...
									var that = this;
									// call the jquery ui sortable on the
									// columns
									this.element
											.sortable({
												handle : ".sDashboardWidgetHeader",
												start : function(event, ui) {
													ui.item.startPosition = ui.item
															.index();
												},
												update : function(event, ui) {
													var newPosition = ui.item
															.index();
													_dashboardData.splice
															.apply(
																	_dashboardData,
																	[
																			newPosition,
																			0 ]
																			.concat(_dashboardData
																					.splice(
																							ui.item.startPosition,
																							1)));
									...
									var disableSelection = this.options
											.hasOwnProperty("disableSelection") ? this.options.disableSelection
											: true;
									if (disableSelection) {
									...
									// trigger creation complete when the
									// dashboard widgets are constructed
								...
								_bindEvents : function() {
									...
									this.element
											.on(
													"click",
													".sDashboardWidgetHeader div.sDashboard-icon.sDashboard-maximize-icon",
													function(e) {
														...
														var widget = $(
																e.currentTarget)
																.parents(
																		".sDashboardWidget:first");
														// get the widget
														// Content
														var widgetContainer = widget
																.find(".sDashboardWidgetContent");

														var widgetDefinition = self
														...
														$(e.currentTarget)
																.toggleClass(
																		"sDashboard-minimize-icon");
														// change the tooltip on
														// the maximize/minimize
														// icon buttons
														if ($(e.currentTarget)
																.attr("title") === "Maximize") {
															$(
															...
															$(e.currentTarget)
																	.attr(
																			...
																			"Minimize");
															$(
																	".sDashboardWidgetHeader div.sDashboard-iconcustomDel.sDashboard-trash-icon ")
																	.hide();
															self
																	._trigger(
																			"widgetMaximized",
														...
														// toggle the class for
														// widget and inner
														// container
														widget
																.toggleClass("sDashboardWidgetContainerMaximized");
														widgetContainer
																.toggleClass("sDashboardWidgetContentMaximized ");

														if (widgetDefinition.widgetType === "chart") {
															var chartArea = widgetContainer
																	.find(" div.sDashboardChart");
															// Flotr.draw(chartArea[0],
															// widgetDefinition.widgetContent.data,
															// widgetDefinition.widgetContent.options);
															var layout = {
																xaxis : {
																	tickangle : -45
																},
									...
									this.element
											.on(
													"click",
													".sDashboardWidgetHeader div.sDashboard-icon.sDashboard-refresh-icon",
													function(e) {
														var widget = $(
																e.currentTarget)
																.parents(
														...
														var widgetDefinition = self
																._getWidgetContentForId(
																		widgetId,
																		self);
														var refreshedData = widgetDefinition.refreshCallBack
																.apply(
																		self,
																		[ widgetId ]);
									...
									// delete widget by clicking the 'trash'
									// icon on the widget
									this.element
											.on(
													"click",
													...
													function(e) {
														...
														widget
																.hide(
																		...
																		function() {
																			self
																					._removeWidgetFromWidgetDefinitions(this.id);
																			$(
																					this)
																					.remove();
																			$(
									...
									this.element
											.on(
													"click",
													".sDashboardWidgetContent table.sDashboardTableView tbody tr",
													function(e) {
														var selectedRow = $(e.currentTarget);

														if (selectedRow.length > 0) {
															var selectedDataTable = selectedRow
																	.parents(
																			'table:first')
																	.dataTable();

															var selectedWidget = selectedRow
																	.parents("li:first");
															var selectedRowData = selectedDataTable
																	.fnGetData(selectedRow[0]);
															var selectedWidgetId = selectedWidget
																	.attr("id");
															var evtData = {
								...
								_constructWidget : function(widgetDefinition) {
									...
									var widgetContainer = $("<div/>").addClass(
											"sDashboardWidget");

									// create a widget header
									/*
									 * var widgetHeader = $("<div/>").addClass("sDashboardWidgetHeader
									 * portlet-title"); var maximizeButton = $('<a
									 * class="btn btn-circle btn-icon-only
									 * btn-default fullscreen
									 * sDashboard-iconcustomStyle
									 * sDashboard-maximize-icon"
									 * data-container="false"
									 * data-placement="bottom"
									 * href="javascript:;"> </a>'); var
									 * settingsButton = $('<a class="btn
									 * btn-circle btn-icon-only btn-default
									 * sDashboard-iconcustomStyle
									 * sDashboard-settings" href="javascript:;"><i
									 * class="icon-wrench"></i></a>'); var
									 * deleteButton = $('<a class="btn
									 * btn-circle btn-icon-only btn-default
									 * sDashboard-iconcustomStyle
									 * sDashboard-trash-icon"
									 * href="javascript:;"><i
									...
									var widgetHeader = $("<div/>")
											.addClass(
													"sDashboardWidgetHeader sDashboard-clearfix");
									...
									if (widgetDefinition
											.hasOwnProperty("enableRefresh")
											&& widgetDefinition.enableRefresh) {
									...
									// add widget title
									widgetHeader
											.append(widgetDefinition.widgetTitle);

									// create a widget content
									var widgetContent = $("<div/>").addClass(
									...
									if (widgetDefinition.widgetType === 'table') {
										...
										var chart = $('<div/>').addClass(
												"sDashboardChart");
										if (widgetDefinition.getDataBySelection) {
											chart
													.addClass("sDashboardChartSelectable");
											// chart.addClass("chart");
										} else {
											chart
													.addClass("sDashboardChartClickable");
										}
										widgetContent.append(chart);
									} else {
										widgetContent
												.append(widgetDefinition.widgetContent);
									}
								...
								_refreshRegularWidget : function(
										widgetDefinition, widget) {
									var isMaximized = widget
											.find(".sDashboardWidgetContent")
											.hasClass(
													'sDashboardWidgetContentMaximized');
									// first remove the content
									widget.find('.sDashboardWidgetContent')
											.empty().remove();
									...
									if (isMaximized) {
										widgetContent
												.addClass('sDashboardWidgetContentMaximized');
									}
									widgetContent
											.append(widgetDefinition.widgetContent);
									// then append this to the widget again;
									widget.find(".sDashboardWidget").append(
											widgetContent);
								},
								_refreshTable : function(widgetDefinition,
										widget) {
									var selectedDataTable = widget.find(
											'table:first').dataTable();
									selectedDataTable.fnClearTable();
									selectedDataTable
											.fnAddData(widgetDefinition.widgetContent["aaData"]);
								...
								_renderTable : function(widgetDefinition) {
									...
									var table
									if (widgetDefinition.widgetType === 'table') {
										table = this.element.find(id
												+ " table.sDashboardTableView");

										var tableDef = {};
										$.extend(tableDef,
												widgetDefinition.widgetContent);
								...
								_renderChart : function(widgetDefinition) {
									...
									var config;
									var chart;
									chartArea = this.element.find(id
											+ " div.sDashboardChart");
									...
									if (widgetDefinition.widgetType === 'chart') {
										...
										// Plotly.newPlot(chartArea[0],
										// widgetDefinition.widgetContent.data,
										// widgetDefinition.widgetContent.layout,widgetDefinition.widgetContent.config);
										/*
										 * Plotly.animate(chartArea[0], { data,
										 * layout } , { transition: { duration:
										 * 800, easing: 'cubic-in-out' }});
										 */
										if (widgetDefinition.chartType === 'pie') {
											...
											Plotly
													.newPlot(
															chartArea[0],
															widgetDefinition.widgetContent.data,
															widgetDefinition.widgetContent.layout,
															widgetDefinition.widgetContent.config);
										}
										if (widgetDefinition.getDataBySelection) {
											this._bindSelectEvent(chartArea[0],
													widgetDefinition.widgetId,
													widgetDefinition, this);
										} else {
											this._bindChartEvents(chartArea[0],
													widgetDefinition.widgetId,
													widgetDefinition, this);
										}
								...
								_bindSelectEvent : function(chartArea,
										widgetId, widgetDefinition, context) {
									Flotr.EventAdapter
											.observe(
													chartArea,
													"flotr:select",
								...
								_bindChartEvents : function(chartArea,
										widgetId, widgetDefinition, context) {
								...
								_removeWidgetFromWidgetDefinitions : function(
										widgetId) {
								...
								_ifWidgetAlreadyExists : function(widgetId) {
									...
									//get the dom element
									var widget = this.element.find("li"
											+ idSelector);
									if (widget.length > 0) {
										return true;
									}
								...
								addWidget : function(widgetDefinition) {
									...
									if (this
											._ifWidgetAlreadyExists(widgetDefinition.widgetId)) {
										this.element
												.find(
														"li#"
																+ widgetDefinition.widgetId)
										...
										this.options.dashboardData
												.unshift(widgetDefinition);
										var widget = this
												._constructWidget(widgetDefinition);
								...
								removeWidget : function(widgetId) {
									...
									if (widget.length > 0) {
										...
										this.element.find("li" + idSelector)
												.remove();
										//remove the dom element from the widgetDefinition
										this
												._removeWidgetFromWidgetDefinitions(widgetId);
									}
								},
